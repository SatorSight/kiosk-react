...
class IssuesSwiper extends React.Component {
    ...
    render() {
        ...
        let styleDot;
        if (active) {
            styleDot = Object.assign({}, styles.captionSwiper, styles.captionSwiperactive);
        } else {
            styleDot = styles.captionSwiper;
        }
        let dot =0;

        return (
            <div style={styles.mainSwiper}>
                <SwipeableViews enableMouseEvents index={index} style={styles.IssuesSwiper} onChangeIndex={this.handleChangeIndex} onSwitching={this.handleSwitch}>
